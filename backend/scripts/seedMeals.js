...
function assignCuisine(foodName) {
  ...
  if (
    ["idli", "dosa", "sambar", "rasam", "vada", "uttapam"].some((word) =>
      name.includes(word),
    )
  ) {
    return "South Indian";
  } else if (
    ["roti", "dal", "chole", "butter", "tandoori", "paratha", "palak"].some(
      (word) => name.includes(word),
    )
  ) {
    return "North Indian";
  } else if (
    ["pani puri", "samosa", "dhokla"].some((word) => name.includes(word))
  ) {
    return "Street Food";
  } else {
    return "General";
...
function assignCategory(foodName, originalCategory) {
  ...
  if (
    ...
    ["idli", "dosa", "upma", "poha", "paratha"].some((word) =>
      name.includes(word),
    )
  ) {
    return "Breakfast";
    ...
    ["samosa", "vada", "dhokla", "pani puri"].some((word) =>
      name.includes(word),
    )
  ) {
    return "Snack";
    ...
    ["sambar", "rasam"].some((word) => name.includes(word))
  ) {
    return "Side Dish";
  } else if (
    category.includes("staple") ||
    ["roti", "rice"].some((word) => name.includes(word))
  ) {
    return "Staple";
  } else {
    return "Main Dish";
...
function resolveCsvPath() {
  const candidates = [
    path.join(__dirname, "../../ml/api/indian_food_nutrition_dataset.csv"),
    path.join(__dirname, "../../ml/indian_food_nutrition_dataset.csv"),
  ];
...
async function seedMeals() {
  try {
    ...
    if (!csvPath) {
      throw new Error(
        "CSV file not found. Please ensure the ML dataset exists.",
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        const servingSize = values[2]?.replace(/"/g, "").trim() || "1 serving";
        ...
        const meal = {
          ...
          dietaryPreference: dietPref.includes("Non")
            ? "Non-Vegetarian"
            : "Vegetarian",
          servingSize,
          description: `Traditional Indian ${assignCuisine(foodName)} dish`,
          isActive: true,
          nutritionScore: Math.min(
            10,
            Math.max(1, Math.round((protein / calories) * 100 * 2)),
          ),
